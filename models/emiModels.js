...
exports.model_add = async (postData) => {
  try {
    ...
    return new Response(200, "T", { list, pegination }).custom(
      authHandler("EMI_ADDED")
    );
  } catch (error) {
    return new Response(400, "F").custom(error.message);
  }
...
exports.model_update = async (postData) => {
  try {
    ...
    if (error) return handleError(400, error.details[0].message);
    const existingEmi = await Emi.findById(value.emi_id);
    if (!existingEmi)
...
exports.model_delete = async (postData) => {
  try {
    return DeleteRecordById(Emi, postData.emi_id, "EMI");
  } catch (error) {
    return new Response(400, "F").custom(error.message);
  }
};
